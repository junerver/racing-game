...
游戏针对后期高速场景进行了全面的性能优化，确保流畅的 60FPS 游戏体验：

### 渲染优化

- **智能渲染系统**：避免不必要的完整重绘，只在对象数量或状态变化时更新
- **可见性剔除**：只渲染屏幕可见范围内的对象，减少绘制开销
- **自适应画质**：根据实时帧率动态调整特效质量
  - FPS < 30：禁用所有特效（最低性能模式）
  - FPS 30-45：保留基本发光效果（中性能模式）
  - FPS > 45：开启所有特效（高性能模式）

### 内存优化

- **对象池系统**：预分配游戏对象，减少垃圾回收压力
- **批处理渲染**：缓存渲染状态，避免重复设置

### 碰撞检测优化

- **空间分区**：只检测车辆附近 200px 范围内的对象
- **早期退出**：碰撞检测使用优化的 AABB 算法

### 帧率稳定性

- **固定时间步长**：锁定游戏逻辑为 60FPS
- **死亡螺旋防护**：单帧最多更新 3 次，避免卡顿连锁反应
- **累积器重置**：防止长时间卡顿后的追赶更新
...
- **操作**：使用 ← → 方向键控制车辆左右移动
- **生命值**：游戏开始时根据车辆类型有 2-4 颗心，每次碰撞失去 1 颗心，失去所有心后游戏结束
- **速度**：车辆速度会随着行驶距离逐渐增加，难度也会相应提升

## 🚗 车辆系统

游戏提供四种不同类型的车辆，每种车辆都有独特的外观、属性和特殊能力：
...
- **极速**：车辆能达到的最高速度
- **操控**：车辆左右移动的灵敏度
- **道具时长**：道具效果持续时间的修正（引擎等级越高越短）
- **稳定性**：操控的稳定程度（轮胎等级越高越灵敏但更难控制）
...
1. 选择四种车辆类型之一
2. 调整引擎等级（1-3 级）：影响加速度和道具持续时间
3. 调整轮胎等级（1-3 级）：影响极速、操控和稳定性
...
游戏使用 SQLite 数据库存储所有游戏数据到服务端，支持数据持久化和多玩家排行榜：

### 主要功能

- **自动玩家身份管理**：首次游戏时自动生成唯一玩家名称并保存到本地
- **游戏记录保存**：游戏结束时自动保存详细数据到服务端数据库
- **全球排行榜**：所有玩家的公共排行榜，显示玩家名称、车辆和成绩
  - 支持按距离和分数排序
  - 实时更新，游戏结束后自动上传
- **玩家统计**：追踪个人游戏历史和成就
- **数据分析**：详细的道具使用和 Boss 战统计
...
**注意**：排行榜数据包含玩家用户名，用于显示所有玩家的公共排行榜。
...
响应示例：
{
  ...
  "data": {
    "connected": true,
    "version": 1,
    "databasePath": "/path/to/racing_game.db",
    "timestamp": 1234567890000
  }
}
```
